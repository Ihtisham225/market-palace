...
class CompanyController extends Controller
{
    ...
    {
        $rows = Company::where('user_id', auth()->user()->id)->latest()->paginate(10);
        $total = count($rows);
    ...
    //save record
    public function save_record(CompanyRequest $request)
    {
        $info = new Company();
    ...
    //update record
    public function update_record(CompanyRequest $request, $id)
    {
        $info = Company::find($id);
        $info->user_id = auth()->user()->id;
        $info->name = $request->name;
    ...
    {
        $rows = Company::onlyTrashed()->where('user_id', auth()->user()->id)->latest()->paginate(10);
        $total = count(Company::withTrashed()->where('deleted_at', '!=', null)->where('user_id', auth()->user()->id)->get());
        $title = "Companies";
        $badge = "trashed";

        return view('admin.company.trashed', [
            'rows' => $rows,
            'total' => $total,
            'title' => $title,
            'badge' => $badge,
    ...
    {
        $info = Company::onlyTrashed()->find($id);
    ...
    public function search_record(Request $request)
    {
        ...
        $rows = Company::where('name', 'LIKE', "%{$request->search}%")->where('user_id', auth()->user()->id)->latest()->paginate(10);
        $total = $rows->count();
        $total_active = count($rows->where('status', 1));
        $total_inactive = count($rows->where('status', 0));
    ...
    public function filter_status(Request $request)
    {
        if($request->status == 1)
        {
            $rows = Company::where('user_id', auth()->user()->id)->where('status', 1)->latest()->paginate(10);
        ...
        {
            $rows = Company::where('user_id', auth()->user()->id)->where('status', 0)->latest()->paginate(10);
