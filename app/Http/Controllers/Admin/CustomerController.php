...
class CustomerController extends Controller
{
    ...
    {
        $rows = Customer::where('shop_id', session('shop_id'))->latest()->paginate(10);
        $total = count($rows);
    ...
    //save record
    public function save_record(CustomerRequest $request)
    {
        $info = new Customer();
    ...
    //update record
    public function update_record(CustomerRequest $request, $id)
    {
        $info = Customer::find($id);
        $info->name = $request->name;
        $info->phone = $request->phone;
    ...
    {
        $rows = Customer::onlyTrashed()->where('shop_id', session()->get('shop_id'))->latest()->paginate(10);
        $total = count(Customer::withTrashed()->where('deleted_at', '!=', null)->where('shop_id', session()->get('shop_id'))->get());
        $title = "Salamans";
        $badge = "trashed";

        return view('admin.customer.trashed', [
            'rows' => $rows,
            'total' => $total,
            'title' => $title,
            'badge' => $badge,
    ...
    {
        $info = Customer::onlyTrashed()->find($id);
    ...
    public function search_record(Request $request)
    {
        ...
        $rows = Customer::where('name', 'LIKE', "%{$request->search}%")->where('shop_id', 'shop_id', session()->get('shop_id'))->latest()->paginate(10);
        ...
        $title = "Customers";
        $badge = "all";
        $shops = Customer::where('shop_id', 'shop_id', session()->get('shop_id'))->where('status', '1')->get();
    ...
    public function filter_status(Request $request)
    {
        if($request->status == 1)
        {
            $rows = Customer::where('shop_id', session()->get('shop_id'))->where('status', 1)->latest()->paginate(10);
        ...
        {
            $rows = Customer::where('shop_id', 'shop_id', session()->get('shop_id'))->where('status', 0)->latest()->paginate(10);
